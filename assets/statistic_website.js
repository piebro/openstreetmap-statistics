...
function add_single_line_plot(plot_title, data_name, options = {}) {
    ...
    show_content = async function (div_id) {
        ...
        Plotly.newPlot(
            div,
            traces,
            get_line_plot_layout(plot_title, opt.x_column_name, y_unit, opt.percent),
            get_plot_config(),
        );
    };
    return {
        show: show_content,
        save_plot: async function (div_id) {
            save_plot(data_name, div_id);
        },
...
function add_multi_line_plot(plot_title, data_name, y_unit, options = {}) {
    ...
    show_content = async function (div_id) {
        ...
        Plotly.newPlot(
            div,
            traces,
            get_line_plot_layout(plot_title, opt.x_column_name, y_unit, opt.percent, bar_chart_on_top_of_each_other),
            get_plot_config(),
        );
    };
    return {
...
function add_map_plot(plot_title, data_name) {
    show_content = async function (div_id) {
        ...
        d = data["data"][0];
        traces = get_map_plot_traces(
            d[data_indices["x"]],
            d[data_indices["y"]],
            d[data_indices["z"]],
            d[data_indices["max_z_value"]],
        );
    ...
    show_content = async function (div_id) {
        ...
        for (const d of data["data"]) {
            ...
            Plotly.newPlot(
                div,
                traces,
                get_map_plot_layout(d[data_indices["map_name"]], d[data_indices["max_z_value"]]),
                get_plot_config(),
            );
        }
    };
