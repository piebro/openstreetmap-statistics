...
def save_data(time_dict, progress_bar, name, pd_df_or_series, columns):
    ...
    if start_index_offset != None:
        value_columns = [c for c in columns if c not in ["months", "years"]]
        if_zero_row = (pd_df_or_series[value_columns] != 0).any(axis=1).values
        start_index = np.max([0, np.nonzero(if_zero_row)[0][0] - start_index_offset])
        pd_df_or_series = pd_df_or_series.loc[start_index:]

    pd_df_or_series[columns].to_json(
        os.path.join("assets", "data", f"{name}.json"), orient="split", index=False, indent=1
    )
...
def save_base_statistics(
    ...
    if edit_count_monthly:
        save_data(
            ...
            ddf.groupby(["month_index"])["edits"].sum().compute(),
            ("months", "edits"),
        )
    ...
    if changeset_count_monthly:
        save_data(
            ...
            ddf.groupby(["month_index"]).size().compute().rename("changesets"),
            ("months", "changesets"),
        )
    ...
    if contributor_count_yearly:
        save_data(
            ...
            ddf.groupby(["year_index"])["user_index"].nunique().compute().rename("contributors"),
            ("years", "contributors"),
        )

    if contributor_count_monthly or new_contributor_count_monthly:
        contributors_unique_monthly = (
            ddf.groupby(["month_index"])["user_index"].unique().compute().rename("contributors")
        )
...
def save_base_statistics_tag(
    ...
    if edit_count_monthly:
        ...
        df = ddf[ddf[tag].isin(indices)].groupby(["month_index", tag])["edits"].sum().compute().reset_index()
...
def get_tag_top_k_and_save_top_k_total(
    ...
    for unit in units:
        ...
        df = pd.DataFrame([total.loc[indices].values], columns=names)
        save_data(
            time_dict,
            progress_bar,
...
def save_tag_top_10_contributor_count_first_changeset_monthly(
    ...
    df_names = pd.read_json(os.path.join("assets", "data", f"{data_name_for_top_names}.json"), orient="split")
...
def get_columns_without_months_and_years(df):
    return [v for v in df.columns.values if v not in ["months", "years"]]


def save_accumulated(data_name):
    df = pd.read_json(os.path.join("assets", "data", f"{data_name}.json"), orient="split")
    columns = get_columns_without_months_and_years(df)
    df[columns] = df[columns].cumsum()
    df.to_json(os.path.join("assets", "data", f"{data_name}_accumulated.json"), orient="split", index=False, indent=1)
...
def save_top_k(k, data_name):
    ...
    df.to_json(os.path.join("assets", "data", f"{new_data_name}.json"), orient="split", index=False, indent=1)


def save_percent(data_name, data_name_divide, merge_on_column_name, divide_column_name):
    df_divide = pd.read_json(os.path.join("assets", "data", f"{data_name_divide}.json"), orient="split")
    df_divide = df_divide.rename(columns={divide_column_name: f"{divide_column_name}_divide"})

    df = pd.read_json(os.path.join("assets", "data", f"{data_name}.json"), orient="split")
    columns = get_columns_without_months_and_years(df)
    merged_df = pd.merge(df, df_divide, on=merge_on_column_name, suffixes=("", "_divide"))
    ...
    merged_df[[merge_on_column_name, *columns]].to_json(
        os.path.join("assets", "data", f"{data_name}_percent.json"),
        orient="split",
        index=False,
        indent=1,
        double_precision=1,
...
def save_monthly_to_yearly(data_name, only_full_years=False):
    assert "contributor" not in data_name  # this can not work for contributors
    df = pd.read_json(os.path.join("assets", "data", f"{data_name}.json"), orient="split")
    assert "months" in df
    ...
    df = df.groupby(["years"]).sum().reset_index()
    if only_full_years and years[-12] != years[-1]:
        df = df[:-1]

    suffix = "_only_full_years" if only_full_years else ""
    df.to_json(
        os.path.join("assets", "data", f"{data_name.replace('_monthly', '_yearly')}{suffix}.json"),
        orient="split",
        index=False,
        indent=1,
    )
...
def save_sum_of_top_k(data_name):
    ...
    new_df.to_json(os.path.join("assets", "data", f"{data_name}_sum_top_k.json"), orient="split", index=False, indent=1)
...
def save_div(save_data_name, data_name, data_name_divide, merge_on_column_name, divide_column_name):
    ...
    merged_df[[merge_on_column_name, *columns]].to_json(
        os.path.join("assets", "data", f"{save_data_name}.json"),
        orient="split",
        index=False,
        indent=1,
        double_precision=2,
...
def save_merged_yearly_total_data(yearly_data_name, total_data_name):
    df_yearly = pd.read_json(os.path.join("assets", "data", f"{yearly_data_name}.json"), orient="split")
    df_total = pd.read_json(os.path.join("assets", "data", f"{total_data_name}.json"), orient="split")
    df_total["years"] = "total"
    df_concat = pd.concat([df_yearly, df_total], ignore_index=True)
    df_concat.to_json(
        os.path.join("assets", "data", f"{yearly_data_name}_total.json"), orient="split", index=False, indent=1
    )
